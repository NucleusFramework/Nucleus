...
case "$HOST_ARCH" in
    x86_64)
        ...
        cp "target/x86_64-unknown-linux-gnu/release/libnucleus_share.so" \
           "$OUT_DIR/libnucleus_share.so"
        strip --strip-unneeded "$OUT_DIR/libnucleus_share.so" || true
        ;;
    aarch64|arm64)
        ...
        cp "target/aarch64-unknown-linux-gnu/release/libnucleus_share.so" \
           "$OUT_DIR/libnucleus_share.so"
        strip --strip-unneeded "$OUT_DIR/libnucleus_share.so" || true
        ;;
    *)
...
ls -lh "$OUT_DIR/libnucleus_share.so"

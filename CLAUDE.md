...
- **Packaging task inputs**: `AbstractElectronBuilderPackageTask` reads `NsisSettings`/`MsiSettings`/etc. file properties (icons, license, header/sidebar bitmaps, scripts) from its `distributions` field, which is `@Internal` for config-generation convenience. Gradle's up-to-date checking can't see through it, so every settings file that affects packaged output needs its own mirrored `@InputFile @Optional` property on the task (see `windowsIconFile` for the pattern), wired from the config function alongside it — otherwise a build after adding or changing one of these files can be considered up-to-date and silently reuse a stale installer.
...
- GraalVM Deb/Rpm/Pacman packages honor `linux { afterInstall / afterRemove / beforeInstall / beforeRemove / afterUpgrade / beforeUpgrade }` the same as JVM jpackage/electron-builder. User scripts are concatenated after Nucleus templates. electron-builder substitutes `${sanitizedProductName}` and `${executable}` only when those tokens are single-quoted (`'${sanitizedProductName}-daemon.service'`); double-quoted `"${sanitizedProductName}"` is left unsubstituted and systemd hooks silently no-op. On Pacman, none of `afterInstall`/`afterRemove`/`beforeInstall`/`beforeRemove` run at all during an upgrade (pacman replacing an already-installed package) — only `afterUpgrade`/`beforeUpgrade` (mapped to fpm's `--after-upgrade`/`--before-upgrade`, raw args with no electron-builder macro substitution) fire then, with no fallback. Register/restart a systemd unit in both `afterInstall` and `afterUpgrade` if it must survive upgrades.

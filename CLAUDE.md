...
- `examples/` - Demo & sample applications (consolidated): `nucleus-demo` (flagship), `tao-demo`, `jni-demo`, `jewel-demo`, `cmp-demo` (KMP), `scheduler-demo`, `service-management-demo`, `system-info-demo`, `fs-watcher-smoke`, `extra-launcher-demo`, `benchmark-demo` (JIT-vs-GraalVM-O3 CPU benchmark suite, with SwiftUI + Tauri ports under `ports/`), plus `shared` (Compose helper used by tao/jni demos)
...
- The GraalVM toolchain is auto-downloaded by default (`graalvm { toolchain { } }` DSL): Oracle GraalVM (former EE) — innovation channel by default (e.g. `25i1` from gds.oracle.com), `channel = GraalvmChannel.LTS` or an explicit `version` ("25", "25.0.1") configurable — cached under `~/.gradle/nucleus/graalvm/`. On Intel macs (dropped by Oracle after 25.0.1) it falls back to Liberica NIK via the BellSoft API. A valid `GRAALVM_HOME` env var always wins. CI relies on this auto-download too (`setup-nucleus` no longer installs GraalVM; it caches `~/.gradle/nucleus/graalvm` keyed by OS/arch/version). `toolchain { autoDownload = false }` restores Gradle toolchain resolution via `javaLanguageVersion`/`jvmVendor`. Oracle GraalVM is required for PGO/`-O3`; GraalVM CE lacks AWT support.

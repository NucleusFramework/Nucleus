...
val buildNativeMacOs by tasks.registering(Exec::class) {
    ...
    val hasPrebuilt =
        nativeResourceDir
            .dir("darwin-aarch64")
            .file("libnucleus_webview_macos.dylib")
            .asFile
            .exists()
...
mavenPublishing {
    coordinates("io.github.kdroidfilter", "nucleus.webview-macos", publishVersion)

    pom {
        name.set("Nucleus WebView macOS")
        description.set(
            "WKWebView interop for Compose Desktop on macOS via JNI, with native title bar overlay support.",
        )

...
# robius-speech builds a Swift bridge that requires macOS 11+, and refuses the
# older default deployment target of the x86_64 Rust toolchain.
...
cp "target/aarch64-apple-darwin/release/libnucleus_speech.dylib" \
   "$OUT_DIR_ARM64/libnucleus_speech.dylib"
strip -x "$OUT_DIR_ARM64/libnucleus_speech.dylib" || true

"$CARGO_BIN" build --release --target x86_64-apple-darwin
cp "target/x86_64-apple-darwin/release/libnucleus_speech.dylib" \
   "$OUT_DIR_X64/libnucleus_speech.dylib"
strip -x "$OUT_DIR_X64/libnucleus_speech.dylib" || true
...
ls -lh "$OUT_DIR_ARM64/libnucleus_speech.dylib"
ls -lh "$OUT_DIR_X64/libnucleus_speech.dylib"

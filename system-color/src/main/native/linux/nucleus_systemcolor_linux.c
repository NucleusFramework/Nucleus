...
static void notify_accent_color_changed(double r, double g, double b) {
    ...
    jclass cls = (*env)->FindClass(env,
        "dev/nucleusframework/systemcolor/linux/NativeLinuxSystemColorBridge");
    if (cls) {
...
Java_dev_nucleusframework_systemcolor_linux_NativeLinuxSystemColorBridge_nativeGetAccentColor(
...
JNIEXPORT jboolean JNICALL
Java_dev_nucleusframework_systemcolor_linux_NativeLinuxSystemColorBridge_nativeIsHighContrast(
...
JNIEXPORT jboolean JNICALL
Java_dev_nucleusframework_systemcolor_linux_NativeLinuxSystemColorBridge_nativeIsAccentColorSupported(
    JNIEnv *env, jclass clazz) {
    (void)env; (void)clazz;
    double r, g, b;
...
Java_dev_nucleusframework_systemcolor_linux_NativeLinuxSystemColorBridge_nativeStartObserving(
    JNIEnv *env, jclass clazz) {
    (void)env; (void)clazz;
...
JNIEXPORT void JNICALL
Java_dev_nucleusframework_systemcolor_linux_NativeLinuxSystemColorBridge_nativeStopObserving(
    JNIEnv *env, jclass clazz) {
    (void)env; (void)clazz;
    if (!g_watching) return;
    g_stopFlag = 1;

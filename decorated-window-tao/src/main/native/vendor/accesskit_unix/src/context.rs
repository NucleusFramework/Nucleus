...
/// Vendored-fork addition: lets host code (e.g. JVM-bridged toolkits) provide
/// an explicit app name before any AT-SPI client connects. Without this, the
/// AppContext defaults to `current_exe()` which on Java apps is just "java".
/// Must be called BEFORE the first Adapter is constructed; later calls are
/// silently ignored (OnceLock semantics).
pub fn set_app_name(name: String) {
    let _ = APP_NAME_OVERRIDE.set(name);
}

fn app_name() -> Option<String> {
    if let Some(name) = APP_NAME_OVERRIDE.get() {

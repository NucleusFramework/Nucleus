...
impl QueuedEvents {
    ...
    pub fn raise(self) {
        // Vendored-fork: do not panic the host process on raise HRESULT
        // failures (upstream used `.unwrap()`). Empty queues are a no-op.
        for event in self.0 {
            match event {
                QueuedEvent::Simple { element, event_id } => {
                    let _ = unsafe { UiaRaiseAutomationEvent(&element, event_id) };
                }

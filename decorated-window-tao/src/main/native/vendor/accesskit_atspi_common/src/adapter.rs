...
impl<'a> AdapterChangeHandler<'a> {
    ...
    fn emit_text_change_if_needed_parent(&mut self, old_node: &Node, new_node: &Node) {
        if !new_node.supports_text_ranges() || !old_node.supports_text_ranges() {
            // Vendored-fork addition: text inputs whose toolkit doesn't
            // populate inline text-boxes (Compose via the Tao bridge) still
            // need text-changed events when the field's value mutates so
            // screen readers can announce typed characters. Fall through to
            // the value-based diff path here.
            self.emit_simple_text_change_if_needed(old_node, new_node);
            return;
        }
        let id = new_node.id();
    ...
    fn emit_simple_text_change_if_needed(&mut self, old_node: &Node, new_node: &Node) {
        if !new_node.is_text_input() || !old_node.is_text_input() {
            return;
        }
        let id = new_node.id();
        if self.checked_text_change.contains(&id) {
        ...
        let old_text = old_node.value().unwrap_or_default();
        let new_text = new_node.value().unwrap_or_default();
        ...
        let mut old_chars = old_text.chars();
        let mut new_chars = new_text.chars();
        let mut prefix_usv_count: i32 = 0;
        let mut prefix_byte_count = 0;
        loop {
            match (old_chars.next(), new_chars.next()) {
                (Some(o), Some(n)) if o == n => {
                    prefix_usv_count += 1;
                    prefix_byte_count += n.len_utf8();
                }
                (None, None) => return,
                _ => break,
            }
        ...
        let suffix_byte_count = old_text[prefix_byte_count..]
            ...
            .zip(new_text[prefix_byte_count..].chars().rev())
            .take_while(|(o, n)| o == n)
            .fold(0usize, |c, (o, _)| c + o.len_utf8());
    ...
    fn emit_text_selection_change(&self, old_node: Option<&Node>, new_node: &Node) {
        if !new_node.supports_text_ranges() {
            // Vendored-fork addition: SimpleText nodes (text inputs without
            // text-runs) still carry a raw_text_selection() set by the host
            // toolkit. Emit `object:text-caret-moved` directly from the raw
            // focus index — Orca uses this to track the caret.
            self.emit_simple_caret_change_if_needed(old_node, new_node);
            return;
        }

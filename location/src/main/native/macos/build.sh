...
cp "target/aarch64-apple-darwin/release/libnucleus_location.dylib" \
   "$OUT_DIR_ARM64/libnucleus_location.dylib"
strip -x "$OUT_DIR_ARM64/libnucleus_location.dylib" || true

"$CARGO_BIN" build --release --target x86_64-apple-darwin
cp "target/x86_64-apple-darwin/release/libnucleus_location.dylib" \
   "$OUT_DIR_X64/libnucleus_location.dylib"
strip -x "$OUT_DIR_X64/libnucleus_location.dylib" || true
...
ls -lh "$OUT_DIR_ARM64/libnucleus_location.dylib"
ls -lh "$OUT_DIR_X64/libnucleus_location.dylib"

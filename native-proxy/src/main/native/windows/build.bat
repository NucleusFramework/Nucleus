@echo off
REM Compiles NucleusProxyBridge.c into per-architecture DLLs (x64 + ARM64).
...
echo.
echo Built DLLs:
if exist "%OUT_DIR_X64%\%LIB_NAME%" echo   %OUT_DIR_X64%\%LIB_NAME%
if exist "%OUT_DIR_ARM64%\%LIB_NAME%" echo   %OUT_DIR_ARM64%\%LIB_NAME%

REM Clear the NativeLibraryLoader cache: it is content-addressed, so the stale
REM copy lives under a fingerprint subdirectory and must be removed recursively.
if defined LOCALAPPDATA (
    set "CACHE_BASE=%LOCALAPPDATA%\nucleus\native"
    if exist "!CACHE_BASE!\win32-x64" (
        del /s /q "!CACHE_BASE!\win32-x64\%LIB_NAME%" >nul 2>nul
        echo Cleared x64 cache
    )
    if exist "!CACHE_BASE!\win32-aarch64" (
        del /s /q "!CACHE_BASE!\win32-aarch64\%LIB_NAME%" >nul 2>nul
